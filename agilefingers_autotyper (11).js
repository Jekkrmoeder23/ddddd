(function () {
  ...
  // WPM lock: calculates exact delay needed so running average stays on target
  // Site formula: WPM = totalChars / 5 / minutesElapsed
  // So: next char should fire at ms = (total+1) * 60000 / (target*5) from runStart
  function charMs() {
    var target  = parseInt(wrange.value) || 120;
    var pct     = (parseInt(varInput.value) || 0) / 100;
    var elapsed = Date.now() - runStart;
    // Time when next char SHOULD be typed to keep WPM exactly on target
    var ideal   = (total + 1) * 60000 / (target * 5);
    var delay   = ideal - elapsed;
    // Add small jitter for naturalness but clamp so we never fall behind
    var jitter  = (60000 / (target * 5)) * pct * (Math.random() * 2 - 1);
    return Math.max(8, delay + jitter);
  }
