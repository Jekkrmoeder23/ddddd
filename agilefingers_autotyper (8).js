(function () {
    ...
    '<div id="at-panel">' +
      ...
      '<div id="at-body">' +
        ...
        '<div class="at-switch-row">' +
          ...
          '<div class="at-sw-track" id="sw-mistake-track" onclick="this.classList.toggle(\'on\')"><div class="at-sw-thumb"></div></div>' +
        '</div>' +
        '<div class="at-switch-row">' +
          '<span class="at-switch-lbl">&#9889; Burst mode</span>' +
          '<div class="at-sw-track" id="sw-burst-track" onclick="this.classList.toggle(\'on\')"><div class="at-sw-thumb"></div></div>' +
        '</div>' +
  ...
  var mistakeTrack   = document.getElementById('sw-mistake-track');
  var burstTrack     = document.getElementById('sw-burst-track');
  ...
  function charMs() {
    ...
    if (burstTrack.classList.contains('on')) {
      if (burstLeft > 0) { burstLeft--; return Math.max(8, ms * 0.4); }
      if (total >= nextBurstAt) {
        burstLeft   = 2 + Math.floor(Math.random() * 4);
        nextBurstAt = total + 8 + Math.floor(Math.random() * 8);
        return Math.max(8, ms * 0.4);
      }
    }
    return ms;
  }
  ...
  // ── MISTAKE LOGIC ─────────────────────────────────────────────────────────
  var mistakesLeft = 0, correctingLeft = 0, nextMistakeAt = 0;
  var WRONG = 'abcdefghijklmnopqrstuvwxyz';
  function randWrongChar() { return WRONG[Math.floor(Math.random() * WRONG.length)]; }
  function scheduleMistake() { nextMistakeAt = total + 10 + Math.floor(Math.random() * 21); }
  ...
  // ── AUTO ENGINE ───────────────────────────────────────────────────────────
  var running = false, timer = null, total = 0;
  ...
  function typeNext() {
    ...
    if (correctingLeft > 0) {
      ...
      correctingLeft--;
      timer = setTimeout(typeNext, charMs() * 1.3);
      return;
    }
    ...
    if (mistakesLeft > 0) {
      var inp3 = findInput();
      if (inp3) { inp3.focus(); typeChar(inp3, randWrongChar()); }
      mistakesLeft--;
      if (mistakesLeft === 0) correctingLeft = 1 + Math.floor(Math.random() * 2);
      timer = setTimeout(typeNext, charMs() * 0.9);
      return;
    }

    // Normal typing — just read current char and fire, no waiting
    ...
    // Schedule mistake if enabled
    if (mistakeTrack.classList.contains('on') && total >= nextMistakeAt && mistakesLeft === 0 && correctingLeft === 0) {
      mistakesLeft = 1 + Math.floor(Math.random() * 2);
      scheduleMistake();
    }
  ...
  startBtn.addEventListener('click', function() {
    ...
    mistakesLeft = 0; correctingLeft = 0; burstLeft = 0;
    nextBurstAt = 8 + Math.floor(Math.random() * 8);
    scheduleMistake();
