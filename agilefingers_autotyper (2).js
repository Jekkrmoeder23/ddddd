(function () {
  ...
  root.innerHTML =
    '<div id="at-panel">' +
      ...
      '<div id="at-body">' +
        ...
        // Start WPM + Variance side by side
  ...
  function charMs(charNum) {
    ...
    if (humanOn) {
      // Human: bell curve with occasional pauses/stumbles
      var r = Math.random();
      if (r < 0.025) return base * (3 + Math.random() * 4);  // thinking pause
      if (r < 0.09)  return base * (1.6 + Math.random() * 1.2); // small stumble
      var g = (Math.random() + Math.random() + Math.random() - 1.5) / 1.5;
      return Math.max(20, base + base * 0.2 * g);
    }
