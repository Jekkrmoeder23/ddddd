(function () {
      ...
      '<div id="at-body">' +
        ...
        '<div style="margin-bottom:10px;padding:10px 0;border-top:1px solid rgba(255,255,255,0.06);border-bottom:1px solid rgba(255,255,255,0.06)">' +
          '<div style="display:flex;justify-content:space-between;margin-bottom:8px">' +
            ...
            '</div>' +
          '</div>' +
          '<div class="at-lbl" style="margin-top:8px">Progress</div>' +
          '<div id="at-prog-wrap" style="height:4px;background:rgba(255,255,255,0.06);border-radius:2px;overflow:hidden;margin-top:4px">' +
            '<div id="at-prog-bar" style="height:100%;width:0%;background:#6366f1;border-radius:2px;transition:width 0.1s linear"></div>' +
          '</div>' +
        '</div>' +
  ...
  var progBar        = document.getElementById('at-prog-bar');
  var exerciseLen    = 0;
  var timeLeft       = 0;
  var correctChars   = 0;
  var totalAttempts  = 0;
  ...
  function charMs() {
    ...
    // Self-correct: if we're ahead of schedule fire faster, if behind fire sooner
    // but clamp generously so it never freezes
    if (runStart > 0 && total > 0) {
      var elapsed = Date.now() - runStart;
      var ideal   = total * base;
      var drift   = elapsed - ideal; // positive = we're slow, negative = fast
      var corrected = base - drift * 0.3; // gentle correction
      var jitter  = base * pct * (Math.random() * 2 - 1);
      return Math.max(8, Math.min(base * 3, corrected + jitter));
    }
    var jitter = base * pct * (Math.random() * 2 - 1);
  ...
  function typeNext() {
    ...
    inp.focus();
    if (runStart === 0) runStart = Date.now();
    typeChar(inp, ch);
    total++;
    totalAttempts++;
    correctChars++;
  ...
  startBtn.addEventListener('click', function() {
    ...
    running = true; total = 0; runStart = 0;
    correctChars = 0; totalAttempts = 0; wpmHistory = []; exerciseLen = 0;
